...
if not sameVocab:
    helper_infer = tf_decode_helper.GreedyEmbeddingHelper(
        ...
        start_tokens=tf.fill([numberArticles], target_start_id),
        end_token=5)
else:
    helper_infer = tf_decode_helper.GreedyEmbeddingHelper(
...
if not sameVocab:
    ...
    stacked_articles_dev = load_obj("stacked_articles_dev")
    stacked_annotations_dev = load_obj("stacked_annotations_dev")
else:
    stacked_articles_train = load_obj("stacked_blended_articles_train")
    stacked_annotations_train = load_obj("stacked_blended_annotations_train")
    stacked_articles_dev = load_obj("stacked_blended_articles_dev")
    stacked_annotations_dev = load_obj("stacked_blended_annotations_dev")

articleVocab = load_obj("/scratch/users/akilesh/GO/data/articleVocabDict")
annotationVocab = load_obj("/scratch/users/akilesh/GO/data/annotationVocabDict")
blendedVocab = load_obj("/scratch/users/akilesh/GO/data/blendedVocabDict")

inv_articleVocab = {v: k for k, v in articleVocab.items()}
if not sameVocab:
    inv_annotationVocab = {v: k for k, v in annotationVocab.items()}
    inv_annotationVocab[1] = " "
    inv_annotationVocab[0] = " "
else:
    inv_annotationVocab = {v: k for k, v in blendedVocab.items()}
    inv_annotationVocab[1] = " "
    inv_annotationVocab[0] = " "   
def getNext():
...
def getInputsFromIndices(indices):
    ...
    labels, labels_lengths = hbatch(targets)
    return [features, labels, features_lengths, labels_lengths, articleIndicators]
saver.restore(sess, "/var/tmp/akilesh/run1/model0.ckpt244402")
# saver.restore(sess, "/var/tmp/akilesh/run2/model0.ckpt39200")
...
for i in range(1):
    ...
    result = sess.run(predictions, {encoder_inputs: inputs[0], decoder_targets: inputs[1], encoder_inputs_length: inputs[2], decoder_targets_length: inputs[3], articleIndicators: inputs[4]})
print([inv_annotationVocab[x] for x in inputs[1][0]])
print([inv_annotationVocab[x] for x in result[0]])
    # print(result)
    # vL = open("trainingLosses.txt", "a")
    # vL.write(str(result) + "\n")
    # vL.close()    
    ...
    # if i%200 == 0:  
    #     save_path = saver.save(sess, "/var/tmp/akilesh/run1/model0.ckpt" + str(i + 28000))
    #     print("Model saved in file: %s" % save_path)
    #     #Calculate validation accuracy
    #     count = 0
    #     losses = []
    #     pbar = tqdm.tqdm(total=len(stacked_annotations_dev)/10)
    #     while count + 10 < len(stacked_annotations_dev):
    #         count = count + 10
    #         inputs = getInputsFromIndices(list(range(count - 10, count)))
